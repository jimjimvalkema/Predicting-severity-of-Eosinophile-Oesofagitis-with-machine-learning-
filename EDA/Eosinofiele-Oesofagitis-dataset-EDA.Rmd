...
for (algo.df in split.data.frame(weka.results,weka.results$Key_Scheme))
  {
  for (run.df in split.data.frame(algo.df,algo.df$Key_Run)) {
        new.row <- data.frame(
          Key_Fold = run.df$Key_Fold,
          Key.run = run.df$Key_Run[1]
          ,algo = run.df$Key_Scheme[1]
          ,acc = mean(run.df$Percent_correct)
          ,speed_test = mean(run.df$UserCPU_Time_testing)
          ,speed_train = mean(run.df$UserCPU_Time_training)
          ,TP = median(run.df$Num_true_positives)
          ,TN = median(run.df$Num_true_negatives)
          ,FP = median(run.df$Num_false_positives)
          ,FN = median(run.df$Num_false_negatives)
          ,Key_Dataset = run.df$Key_Dataset
          )
        confusion.df <- rbind(confusion.df,new.row)
    }
}

confusion.df[,2] <- revalue(confusion.df[,2],
                            c("weka.classifiers.bayes.NaiveBayes"="NaiveBayes"
                              ,"weka.classifiers.functions.Logistic"="Logistic"
                              ,"weka.classifiers.lazy.IBk"="IBk"
                              ,"weka.classifiers.rules.OneR"="OneR"
                              ,"weka.classifiers.rules.ZeroR"="ZeroR"
                              ,"weka.classifiers.trees.J48"="J48"
                              ,"weka.classifiers.trees.RandomForest"= "RandomForest"))

for (fold.df in split.data.frame(confusion.df,confusion.df$Key_Fold)) {
        new.row <- data.frame(
          Key.run = fold.df$Key_Run[1]
          ,algo = fold.df$Key_Scheme[1]
          ,acc = mean(fold.df$Percent_correct)
          ,speed_test = mean(fold.df$UserCPU_Time_testing)
          ,speed_train = mean(fold.df$UserCPU_Time_training)
          ,TP = median(fold.df$Num_true_positives)
          ,TN = median(fold.df$Num_true_negatives)
          ,FP = median(fold.df$Num_false_positives)
          ,FN = median(fold.df$Num_false_negatives)
          ,Key_Dataset = fold.df$Key_Dataset
          )
        confusion.folds.df <- rbind(confusion.folds.df,new.row)
    }

for (dataset in split.data.frame(confusion.df,confusion.df$Key_Dataset)){
  dataset 
  print(kable(dataset))
  cat("\n")
  }
