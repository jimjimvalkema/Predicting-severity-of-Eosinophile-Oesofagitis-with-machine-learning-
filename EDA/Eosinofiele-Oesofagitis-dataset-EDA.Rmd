...
*Figure 2: Shows the density of the values of a selection of attributes from the data set. With the density on the y axis and the values on the x axis. This plot clearly shows that the ranges differ drammaticly and because and are skewed to the right which indicated that it could bennefit from a log transformation.*   
...
*Figure 3: same as figure 2 but with then log two transformed. It hows the density of the values of a selection of attributes from the data set. With the density on the y axis and the values on the x axis.*   


### Normalisation
The data varies a lot in its range per each attribute (figure 4) so normalisation is required. Scaling normalisation seems to be optimal since it is able to put the data in more reasonable ranges while still being able to accept new data even if it falls out of the range previously measured unlike min-max normalisation. It can also be more clearly observed that the data represent a more normal distrubtion from log transformation in figure 6. This is where the data is plotted in a density plot like figure 2. This is because the range are now more simmilair becuase of the scaling normalisation.    
...
*Figure 4: Shows a selection of attributes and their distrubutions as a boxplot. The x axis shows the attributes and y axis its values those attributes range within. This plot shows that the range vary largely between attributes.*   
```{r echo=FALSE, cache=FALSE, results=TRUE, warning=FALSE, comment=FALSE, warning=FALSE}
...
*Figure 5: Shows a selection of attributes and their distrubutions after scaling normalisation as a boxplot. The x axis shows the attributes and y axis its values those attributes range within. This plot shows the distrubutions are far more comparable then without normalisation.*   
...
*Figure 6: Shows the density of the values of a selection of attributes from the data set. With the density on the y axis and the values on the x axis. This plot shows that the data is now far more normal distrubuted after scaling and more comparable in its ranges then in figure 2*   
...
*Figure 9: Shows the distribution of the peak max eosinophils whith the peak eosinophils on the x axis and its frequency on te y axis. This plot shows that most values at approxiamtly at 50 and 100  with the rest inbetween.*    
...
ensamble members (all default weka settings): J48, Random Forest, naive Bayes and logistic regression.   
...
These methods are: 
classiefier attribute evaluation with j48. Done with the setting leave one out turned on and with it off.   
And a dataset is created with correlation-based feature selection. The classiefier attribute evaluation data sets also have different variations on when a attribute is no longer included based on rank.   
